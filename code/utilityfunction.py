...
def SMOCU(x, model, xspace):
    ...
    if softtype == 1:  ##soft MOCU with softmax
        ...
        obc_correct = special.logsumexp(k * pymat,
                                        axis=1) / k  ##################################################################################
...
def D_SMOCU(x, model, data):
    smocu2 = 0
    py_x = model.predict_proba(x)
    xspace = model.XspaceGenerate(x_num, data.xspace())
...
def U_SMOCU_K(x, model, k, softtype, x_num, approx_label, version,
              data):  ########### delete xinterval           ################### model.xinterval model.generatexspace
    ...
    return D_SMOCU(x, model, data)  # this equals to E_x' logsumexp(p(y'|x', x, y))


def U_SMOCU(softtype=0, k=1, x_num=1000, approx_label=False, data=None):
    return lambda x, model, version='numpy': U_SMOCU_K(x, model, k, softtype, x_num, approx_label, version, data=data)
...
def D_SMOCU_Approx4(x, model):
    ...
    X_ = model.gpc.X
    kstar = model.gpc.kern.K(xspace, x) - model.gpc.kern.K(xspace,
                                                           X_) @ model.gpc.posterior.woodbury_inv @ model.gpc.kern.K(X_,
                                                                                                                     x)  # size 1*1000
    # print(time() - start)
    ...
    for i in range(2):
        ...
        sigma2_hat = sigma2_ - (sigma2_ ** 2) * norm.pdf(z) / ((1 + sigma2_) * norm.cdf(z)) * (
                    z + norm.pdf(z) / norm.cdf(z))
...
def U_MES(x, model, data):
    py_x = model.predict_proba(x)
    obj = entropy(py_x, axis=1)
    return obj


def U_UCB(x, model, mean=.5, beta=1):
    py_x = model.predict_proba(x)
    obj = -np.abs(py_x[:, 0] - mean) + beta * np.sqrt(py_x[:, 1])
    return obj

def U_UCBS(x, model, mean=.5, beta=1):
    py_x = model.predict_proba(x)
    obj = -np.square(np.abs(py_x[:, 0] - mean)) + beta * np.sqrt(py_x[:, 1])
    return obj

def U_UCBR(x, model, mean=.5, beta=1):
    py_x = model.predict_proba(x)
    obj = -np.sqrt(np.abs(py_x[:, 0] - mean)) + beta * np.sqrt(py_x[:, 1])
    return obj


def U_RANDOM(x, model, data):
    obj = 0
    return obj
